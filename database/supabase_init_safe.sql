-- Script SEGURO para Supabase (maneja objetos existentes)
...
-- Índices para mejorar rendimiento (ignora si ya existen)
...
DROP TRIGGER IF EXISTS update_organizaciones_fecha_actualizacion ON lpdp.organizaciones;
DROP TRIGGER IF EXISTS update_usuarios_fecha_actualizacion ON lpdp.usuarios;
DROP TRIGGER IF EXISTS update_actividades_fecha_actualizacion ON lpdp.actividades_tratamiento;
...
-- Insertar datos semilla solo si no existen
INSERT INTO lpdp.modulos_capacitacion (codigo, titulo, descripcion, orden_secuencia, duracion_estimada) 
VALUES
...
DROP POLICY IF EXISTS "Usuarios pueden ver su propia organización" ON lpdp.organizaciones;
DROP POLICY IF EXISTS "Usuarios pueden ver otros usuarios de su organización" ON lpdp.usuarios;
...
DO $$
BEGIN
    RAISE NOTICE 'Script ejecutado exitosamente. Base de datos configurada.';
END $$;